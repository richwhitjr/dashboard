...
import { usePrioritizedNotion, useRefreshPrioritizedNotion, useDismissPrioritizedItem, useCreateIssue, useAllNotion } from '../api/hooks';
...
export function NotionPage() {
  ...
  return (
    <div>
      ...
      {tab === 'all' && (
        <>
          {allQuery.isLoading && <p className="empty-state">Loading pages...</p>}
          {!allQuery.isLoading && allPages.length === 0 && (
            <p className="empty-state">No synced Notion pages yet. Run a sync to populate.</p>
          )}
          <div>
            {allPages.map((page) => {
              const isExpanded = expandedIds.has(page.id);
              const hasSnippet = !!page.snippet;
              return (
                <div key={page.id} className="dashboard-item-row">
                  <a
                    className="dashboard-item dashboard-item-link"
