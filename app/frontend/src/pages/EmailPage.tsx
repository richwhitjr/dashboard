...
import { usePrioritizedEmail, useRefreshPrioritizedEmail, useDismissPrioritizedItem, useCreateIssue, useAllEmails } from '../api/hooks';
...
export function EmailPage() {
  ...
  return (
    <div>
      ...
      {tab === 'all' && (
        <>
          {allQuery.isLoading && <p className="empty-state">Loading emails...</p>}
          {!allQuery.isLoading && allEmails.length === 0 && (
            <p className="empty-state">No synced emails yet. Run a sync to populate.</p>
          )}
          <div>
            {allEmails.map((email) => {
              const isExpanded = expandedIds.has(email.id);
